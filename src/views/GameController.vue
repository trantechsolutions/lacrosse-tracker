<template>
  ...
  const newPenalty = ref({
    team: 'home', player: '', duration: 30, category: 'crosscheck', releasable: true
  })
  ...
  function adjustTimeout(team, amount) {
    timeouts.value[team] = Math.max(0, Math.min(2, timeouts.value[team] + amount))
    updateFirebase()
  }
  ...
  function editName(type) {
    const name = prompt(`Set ${type} Team Name:`, type === 'home' ? home.value : away.value)
  ...
  function addPenalty() {
    ...
    newPenalty.value = { team: 'home', player: '', duration: 30, category: 'crosscheck', releasable: true }
  }
  ...
  function addPlayerStat() {
    const stat = { ...newPlayerStat.value, gameClock: gameClock.value, quarter: quarter.value }
